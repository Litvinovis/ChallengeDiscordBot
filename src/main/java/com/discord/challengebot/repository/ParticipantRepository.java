...
public class ParticipantRepository {
    ...
    private Participant mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        if (joinDate != null && !joinDate.isBlank()) {
            try { p.setJoinDate(LocalDateTime.parse(joinDate)); }
            catch (Exception e) { log.warn("Не удалось распарсить join_date для участника {}: {}", p.getUserId(), joinDate); }
        }
        ...
        if (lastActivity != null && !lastActivity.isBlank()) {
            try { p.setLastActivityDate(LocalDate.parse(lastActivity)); }
            catch (Exception e) { log.warn("Не удалось распарсить last_activity_date для участника {}: {}", p.getUserId(), lastActivity); }
        }
    ...
    private static Set<String> fromJsonToSetString(String json) {
        ...
        try { return MAPPER.readValue(json, new TypeReference<Set<String>>() {}); }
